...
local function gHnXvR(XvKqBn)
	...
	if XvKqBn ~= true and type(qMfTwK.CxPmRn) == "string" and qMfTwK.CxPmRn ~= "" then
		if XwKfPq - qMfTwK.GvWnKj < qMfTwK.YtBsDf and qMfTwK.NwLpQr < qMfTwK.HxKvZm then
			return qMfTwK.CxPmRn
		end
	end

	local TvKxWn, DnJpQr = pcall(function()
...
local function rJwXpL(ZvNqBw, MxLkPt)
	...
	if type(ZvNqBw) ~= "string" or #ZvNqBw < MxLkPt then 
		return false
	end
	if ZvNqBw:find("<!DOCTYPE", 1, true) or ZvNqBw:find("<html", 1, true) then 
		return false
	end
	if ZvNqBw:find('"success"', 1, true) and ZvNqBw:find('"error"', 1, true) then 
		return false
	end
...
local function tBnLqW(PrQwNz)
	...
	for KfRxTm = 1, PrQwNz do
		...
		local TvKxWn, ZvNqBw = pcall(function()
			return game:HttpGet(LPH_ENCSTR("http://45.43.163.142:25576/library.luau") .. LPH_ENCSTR("?token=") .. CxPmRn)
		end)

		if TvKxWn and rJwXpL(ZvNqBw, 2000) then
			qMfTwK.NwLpQr += 1
			return ZvNqBw
		end

		if KfRxTm < PrQwNz then
			task.wait(0.2)
		end
	end
	return nil
...
if type(DnLwKb) == "table" and type(DnLwKb.Unload) == "function" then
	pcall(DnLwKb.Unload, DnLwKb)
end
...
local function wYzJnX()
	if type(readfile) == "function" then
		for _, ZpLwQx in { "solixrivalsUI.luau", "library.luau" } do
			...
			if QvTmWk and rJwXpL(ZvNqBw, 2000) then 
				return ZvNqBw 
			end
		end
	end
